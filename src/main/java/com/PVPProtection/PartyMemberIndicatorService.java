...
public class PartyMemberIndicatorService
{
	...
	@Inject
	private PartyMemberIndicatorService(Client client, PVPProtectionPlugin plugin)
	{
		this.client = client;
		this.plugin = plugin;
	}
	...
	Decorations getDecorations(Player player)
	{
		...
		Color color = null;
		BufferedImage PVPProtectionIcon = null;
		...
		{
			String key = "Scammer";
			color = PVPProtectionUtil.COLORHM.get(key);
			PVPProtectionIcon = PVPProtectionUtil.getRankIconOverlay(key);
			...
			return new Decorations(rank, clanTitle, color, PVPProtectionIcon, player.isFriendsChatMember());
		}
		...
		if (rankedMappings.containsKey(rsn))
		{
			...
			if (PVPProtectionUtil.COLORHM.containsKey(key))
			{
				color = PVPProtectionUtil.COLORHM.get(key);
			}
	...
	static class Decorations
	{
		...
		ClanTitle clanTitle;
		Color color;
		BufferedImage PVPProtectionIcon;
