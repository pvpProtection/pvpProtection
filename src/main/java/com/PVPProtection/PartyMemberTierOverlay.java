...
public class PartyMemberTierOverlay extends Overlay
{
	...
	@Inject
	private PartyMemberTierOverlay(PVPProtectionPlugin plugin, PVPProtectionConfig config, PartyMemberIndicatorService playerIndicatorsService, ChatIconManager chatIconManager)
	{
		this.plugin = plugin;
		this.config = config;
	...
	private void renderPlayerOverlay(Graphics2D graphics, Player actor, PartyMemberIndicatorService.Decorations decorations)
	{
		...
		else
		{
			...
			BufferedImage dmScamIcon = decorations.getPVPProtectionIcon();

			if (dmScamIcon != null)
			{
				final int imageWidth = dmScamIcon.getWidth();
				final int imageTextMargin;
				final int imageNegativeMargin;
				...
				final Point imageLocation = new Point(textLocation.getX() - imageNegativeMargin - 8, textLocation.getY() - textHeight / 2 - dmScamIcon.getHeight() / 2);
				OverlayUtil.renderImageLocation(graphics, imageLocation, dmScamIcon);
				// move text
				textLocation = new Point(textLocation.getX() - 5 + imageTextMargin, textLocation.getY());
			}
		}
