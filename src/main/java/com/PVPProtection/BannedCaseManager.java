...
public class BannedCaseManager
{
	...
	private static final HttpUrl PVPProtection_FAST_BAN_LIST = HttpUrl.parse("https://pvpprotection.rs/api/bans.json");

	// This list is the github-pages deployed copy of the live data, has a slight delay but better than regular github lists
	private static final HttpUrl PVPProtection_DEFAULT_BAN_LIST = HttpUrl.parse("https://pvpprotection.github.io/pvpProtectionLists/bans.json");
	...
	public void refresh(Runnable onComplete, boolean useLiveList)
	{
		...
		if (useLiveList)
		{
			rwReq = new Request.Builder().url(PVPProtection_FAST_BAN_LIST).addHeader("Cache-Control", "no-cache").build();
		}
		else
		{
			rwReq = new Request.Builder().url(PVPProtection_DEFAULT_BAN_LIST).addHeader("Cache-Control", "no-cache").build();
		}
	...
	/**
	 * Get a PVPProtection case from the cached list
	 *
	 * @param rsn
	 * @return
	 */
