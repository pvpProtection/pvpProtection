...
@ConfigGroup(PVPProtectionConfig.CONFIG_GROUP)
public interface PVPProtectionConfig extends Config
{
	String CONFIG_GROUP = "PVPProtection";
	...
	@ConfigItem(
		position = 12,
		keyName = "openNav",
		name = "Open side panel on challenge",
		description = "Opens PVPProtection side panel when a challenge is issued",
		section = SIDEPANEL_SECTION
	)
	default boolean openPVPProtectionSidePanelOnChallenge()
	{
		return true;
	}
	...
	@ConfigItem(
		...
		description = "Reminder to join PVPProtection discord",
		section = NOTIFICATIONS_SECTION
	)
	...
	@ConfigItem(
		...
		warning = "This will make a connection with the PVPProtection list to pull live data, a website not controlled or maintained by Jagex or RuneLite",
